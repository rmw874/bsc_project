...
from metrics import calculate_dice_per_class, calculate_iou_per_class, plot_metric_heatmap, calculate_tp_fp_fn, calculate_mean_iou, calculate_mean_dice, calculate_mean_pixel_accuracy
...
albumentations_transform = A.Compose([
    A.Affine(rotate=(-2, 2), p=0.5, mode=cv2.BORDER_CONSTANT, cval=255),
    A.Affine(shear=(-2, 2), p=0.5, mode=cv2.BORDER_CONSTANT, cval=255),
])
...
def visualize_batch(epoch, img_tensor, mask_tensor, outputs, loss, criterion):
    ...
    with torch.no_grad():
        ...
        plt.savefig(f'results/dice_ugly/train_epoch_{epoch+1}_lr_{LEARNING_RATE}_bs_{BATCH_SIZE}.png')
        plt.close()
...
def main():
    ...
    plt.tight_layout()
    plt.savefig(f"results/dice_ugly/training_metrics_norm_lr_{LEARNING_RATE}_bs_{BATCH_SIZE}.png")
    plt.close()
    ...
    plt.grid(True, linestyle="--", alpha=0.6)
    plt.tight_layout()
    plt.savefig(f"results/dice_ugly/training_metrics_lr_{LEARNING_RATE}_bs_{BATCH_SIZE}.png")
    plt.close()
    ...
    plt.grid(True, linestyle="--", alpha=0.6)
    plt.tight_layout()
    plt.savefig(f"results/dice_ugly/Mean_IOU_lr_{LEARNING_RATE}_bs_{BATCH_SIZE}.png")
    plt.close()
