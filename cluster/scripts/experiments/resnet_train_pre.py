...
from scripts.validation import validate_epoch
...
def visualize_batch_with_boundaries(
    epoch, img_tensor, mask_tensor, outputs, loss, criterion, N_CLASSES, post_process=False
):
    """
...
def main():
    ...
    for epoch in range(EPOCHS):
        ...
        if current_instance_sep > best_instance_sep:
            ...
            best_epoch = epoch
            # Save both the best model and a checkpoint with epoch information
            checkpoint = {
                'epoch': epoch,
                'model_state_dict': model.state_dict(),
                'optimizer_state_dict': optimizer.state_dict(),
                'instance_sep': best_instance_sep,
                'val_metrics': val_metrics,
            }
            torch.save(checkpoint, f"results/{RUN_NAME}/best_model_checkpoint.pth")
            torch.save(model.state_dict(), f"results/{RUN_NAME}/best_model.pth")
            print(f"Saved best model for epoch {epoch + 1} with instance separation score: {best_instance_sep:.4f}")

    print(f"\nTraining completed. Best model was from epoch {best_epoch + 1} with instance separation score: {best_instance_sep:.4f}")
    ...
    # boundaries
    boundary_f1s_cpu = [f1.cpu().item() if torch.is_tensor(f1) else float(f1) for f1 in boundary_f1s]
    instance_seps_cpu = [sep.cpu().item() if torch.is_tensor(sep) else float(sep) for sep in instance_seps]
    
    epochs = list(range(1, len(boundary_f1s) + 1))
    data = pd.DataFrame({
        "Epoch": epochs * 2,
        "Value": boundary_f1s_cpu + instance_seps_cpu,
        "Metric": ["Boundary F1"] * len(epochs) + ["Instance Separation"] * len(epochs),
    })
    ...
    plt.grid(True, linestyle="--", alpha=0.6)
    plt.tight_layout()
    plt.savefig(f"results/{RUN_NAME}/boundary_metrics_lr_{LEARNING_RATE}_bs_{BATCH_SIZE}.png")
    plt.close()
