...
from post_process import post_process_mask, remove_small_regions, separate_tall_regions, erode_regions, create_bounding_boxes
import re
...
def clean_year_column(df):
    ...
    df['Year'] = df['Year'].replace('', pd.NA).fillna(method='ffill')
    
    return df

def merge_page_dataframes(top_df, bottom_df):
    """
    Merge top and bottom page DataFrames, filling NaN values from the row above.
    """
...
def save_matrix_outputs(df, output_path, base_filename):
    ...
    txt_path = os.path.join(output_path, f"{base_filename}_matrix.txt")
    with open(txt_path, 'w') as f:
        f.write(df.to_string(index=False))
    
    return csv_path, txt_path

def process_and_merge_pages(data_dir, out_dir, matrix_dir, model_img, model_ocr, processor, single_debug=False):
    """
    Process and merge pages with option for single debug mode.
    ...
    for top_path, bottom_path in page_pairs:
        ...
        csv_path, txt_path = save_matrix_outputs(merged_df, matrix_dir, base_filename)
...
def merge_all_csvs(csv_paths, output_dir):
    ...
    merged_path = os.path.join(output_dir, "merged_database.csv")
...
#     # Configure paths
#     data_dir = "data/processed/Mathiesen-single-pages/testing/"
#     out_dir = "results/ocr_raw_large_test/row_col_crops"
#     matrix_dir = "results/ocr_raw_large_test/matrices"
...
def main():
    ...
    generated_csvs = process_and_merge_pages(
        ...
        model_ocr,
        processor,
        single_debug=args.single_debug
    )

    # Merge all CSVs if requested
    if args.merge and generated_csvs:
        merged_path = merge_all_csvs(generated_csvs, args.matrix_dir)
        if merged_path:
